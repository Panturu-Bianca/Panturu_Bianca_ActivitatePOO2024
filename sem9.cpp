...
public:
	...
	Librarie(const char* nume, int nrCarti) :Magazin() {
		this->nume = new char[strlen(nume) + 1];//this->nume parametru
	...
	Librarie(const char* nume, int nrCarti, int nrAngajati, string oras, float* salarii, float suprafata, const int anDeschidere) :Magazin(oras, nrAngajati, suprafata, anDeschidere) {
		...
		strcpy_s(this->nume, strlen(nume) + 1, nume);
		this->nrCarti = nrCarti;
		this->setNrAngajati(nrAngajati, salarii);//librarie primeste un param salarii care nu apare in const Magazin
	}
	~Librarie() {
		if (this->nume)
			delete[]this->nume;
	...
	operator int() {//supraincaracre la op de cast la int
		return this->nrCarti;
	}

	float getNrMediuCartiPerAngajat() {
		float rezultat = this->nrCarti / (float)this->getNrAngajati();//sa dea rezult in float
		return rezultat;
	}
	Librarie operator=(const Librarie& l) { //: Magazin(l) dupa : este lista de initializari a constr 
		if (&l != this) {//verif de autoasig
			Magazin::operator=(l);//op = din cls parinte
			if (this->nume) {
				delete[]this->nume;
			}
			this->nume = new char[strlen(l.nume) + 1];
...
int main() {
	...
	float* salarii = new float[3] {2000, 3000, 4000};// pt ca am pus{} avem initializ inline
	Librarie l3("Daco", 80, 3, "Bucuresti", salarii, 200, 2019);

	Librarie l4(l3);//constr de copiere

	Librarie l5 = l3;//constr de copiere

	Librarie l6;
	l6 = l3;//operat =

	cout << l3;
	cout << l6;//nu o sa fie lfl ca l3 pt ca nu acopiat si atributele specif pt magazin pt ca in op = nu am apelat op = si din cls parinte

	int nrCarti = (int)l3;//apel op de cast la int din magazin
	cout << "nr carti:" << nrCarti << endl;

	cout << "Numarul mediu de carti per anagajat:" << l3.getNrMediuCartiPerAngajat() << endl;
	delete[]salarii;
	return 254;
}